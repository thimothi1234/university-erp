...
<div class="sl-mainpanel">
  ...
  <option value="other">Advance Settlement</option>
  <option value="oother">Payment Against Commitment</option>
...
<select id="other"  name="project" class="form-control" required="required">
<option value="">--Select the Advance--</option>
    @foreach($adv as $adv)

    <option value="{{$adv->id}}">{{$adv->sum}}---{{$adv->vendors->name}}----{{$adv->transactiondate}}</option>
    @endforeach

</select>

<select id="oother"  name="com" class="form-control" required="required">
<option value="">--Select the Commitment--</option>
    @foreach($com as $com)

    <option value="{{$com->id}}">COM-00{{$com->sub}}---{{$com->sum}}</option>
    @endforeach
...
document.querySelector("#dropdown").addEventListener("change", function(e) {
  const other = document.querySelector("#oother");
  if (e.target.value === "oother") {
    other.style.display = "block";
  } else {
    other.style.display = "none";
  }
