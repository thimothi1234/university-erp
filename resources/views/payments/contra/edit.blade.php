...
<div class="sl-mainpanel">
      <nav class="breadcrumb sl-breadcrumb">
        <a class="breadcrumb-item" href="{{route('journal.index')}}">Voucher</a>
        <a class="breadcrumb-item" href="{{route('journal.index')}}">Journal</a>
        <span class="breadcrumb-item active">Create</span>
								...
								<div class="col-lg-12">
									...
									<div class="card card-default">
										...
										<div class="card-body">
                                        <form action="{{route('journal.update',$user->id)}}" method="POST" enctype="multipart/form-data">
...
<!-- 
    ...
    <div class="col-sm">
                ...
                <tr>
                <td style="display:none;"><input type="HIDDEN" name="addmore[{{$ledger->id}}][id]" value="{{$ledger->id}}" class="form-control" required="required"/></td>  
                ...
                @foreach($bud as $iit_bank)<option value="{{$iit_bank->id}}">{{$iit_bank->name}}</option>@endforeach
                </select></td>  
              
<td><select name="addmore[{{$ledger->id}}][cr_dr]" id="" class="cr_dr form-control" required="required">
...
<input type="hidden" value='0' name='addmore[{{$ledger->id}}][banks]' >
...
<input type="hidden" value='Journal' name='debit_credit' >
</td>  
...
<input type="hidden" value="{{ auth()->user()->id }}" name="entered" readonly>
  ...
  <a href="{{route('contra.index')}}" class="btn btn-danger">Cancel</a>
</form>
</div></div></div>
...
$("#add12").click(function(){
    ++i;
    $("#dynamicTable12").append('<tr><td><select name="addmor['+i+'][ledger]" id="" class="form-control js" required="required"><option value="">--Please select--</option>@foreach($bud as $iit_bank)<option value="{{$iit_bank->id}}">{{$iit_bank->name}}</option>@endforeach</select></td><td><select name="addmor['+i+'][cr_dr]" id="" class="cr_dr form-control" required="required"><option value="">--select--</option><option value="Dr">Dr</option><option value="Cr">Cr</option></select></td><td><input type="number" name="addmor['+i+'][amount]" placeholder="Amount" class="txt form-control" required="required"/></td><td><button type="button" class="btn btn-danger remove-tr">x</button></td></tr>');
