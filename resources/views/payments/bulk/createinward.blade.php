...
<div class="sl-mainpanel">
    ...
    <div>
                ...
                <tr>
                    <th scope="col"width="500" >Beneficiary</th>
                    <th scope="col" width="150">Gross</th>
                    <th scope="col" width="150">Income Tax</th>
                    <th scope="col" width="150">Professional Tax</th>
                    <th scope="col" width="150">Net</th>
                </tr>
                <input type="hidden" name="jil" value="01">

                
                @foreach($iid as $iid)

                <input type="hidden" name="inwardids[]" value="cea{{$iid}}cea">



                <tr>
                <td> <select name="addmore1[0][name]" id="" class="form-control js" required="required">
                <?php $led =  \App\Models\cea::where(['id' => $iid])->first() ;?>
                <option value="">{{$led->nmmms->name}}</option>
             
                </select></td>  
                <td><input class="form-control product_price" type="number" id='product_price_{{ $loop->iteration }}' name="addmore1[$loop->iteration][gross]" value="{{$led->amount}}"  for="{{ $loop->iteration }}" required="required"/></td>  
                <td><input class="form-control quantity" type="number" id='quantity_{{ $loop->iteration }}' name="addmore1[$loop->iteration][tds]" placeholder="Income Tax"  for="{{ $loop->iteration }}" value="0" required="required"/></td>  
                <td><input class="form-control product_pricce" type="number" id='product_pricce_{{ $loop->iteration }}' name="addmore1[$loop->iteration][ptax]" placeholder="Professional Tax" for="{{ $loop->iteration }}" value="0" required="required"/></td>  
                <td>
                <input class="form-control total_cost" type="text" id="total_cost_{{ $loop->iteration }}" name="addmore1[$loop->iteration][net]" for="{{ $loop->iteration }}" readonly/></td>
                </td>  
                  @endforeach<td><button type="button" name="add" id="add" class="btn btn-success">+</button></td> </tr>
                </table>
                <table>
                <tr>
              <td scope="col"width="510" style="align:right">Totals</td>
              ...
              <td scope="col"width="120"><input class="form-control nett23" type='text' id='nett23' name='subtotal' readonly/></td>
                ...
                <td>  <select name="addmore[0][sub]" class="sub form-control js" id="city0">
                  <option value="">-- Select Head --</option>
                        ...
                        <tr>
                <td> <select name="addmore[1][ledger]" id="" class="form-control js" >
                <option value="5442">Personal Income Tax 31.2%</option>
